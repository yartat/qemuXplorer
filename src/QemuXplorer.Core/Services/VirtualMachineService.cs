...
public sealed class VirtualMachineService
{
    ...
    /// <summary>Raised when a running VM process exits. May fire on a thread-pool thread.</summary>
    public event EventHandler<Guid>? VmExited
    {
        add => _processManager.VmExited += value;
        remove => _processManager.VmExited -= value;
    }

    public Task<IReadOnlyList<VirtualMachine>> GetAllAsync(CancellationToken ct = default)
        => _vmRepo.GetAllAsync(ct);

    public Task<QemuInstallation?> GetDefaultInstallationAsync(CancellationToken ct = default)
        => _qemuRepo.GetDefaultAsync(ct);
